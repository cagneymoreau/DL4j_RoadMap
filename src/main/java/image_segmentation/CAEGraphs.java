...
/**
 *
 * Ready made convolutional autoencoders
 *
 */
...
public class CAEGraphs {





    // TODO: 12/22/2020 probably easy to break with wrong vars, max inner dims is 128
    // Reduces square input at gemoetric rate.
    //region---------------------shortgraph
     ...
     * @param center
     */
    public static ComputationGraph grabShortEncoder(int feedDim, int baseK, int maxK, int center, double lr, int scoreDisp)
    {
        ...
        List<Triple<String, Layer, String>> inputlayers = inputShortLayers("", layercount, maxK, baseK, 0);
        List<Triple<String, Layer, String>> centerLayers = centerShortLayers(center);
        List<Triple<String, Layer, String>> outputLayers = outputShortLayers(layercount, baseK, maxK, true, 0);
        ...
        net.init();
        net.setListeners(new ScoreIterationListener(scoreDisp));
    ...
    private static List<Triple<String, Layer, String>> inputShortLayers(String combo, int layerCount, int maxK, int baseK, double drop)
    {
    ...
    //128 = 8x4x4
    private static List<Triple<String, Layer, String>> centerShortLayers(int centerDim)
    {
        List<Triple<String, Layer, String>> layerList = new ArrayList<>();
    ...
    private static List<Triple<String, Layer, String>> outputShortLayers(int layerCount, int baseK, int maxK, boolean includeLoss, double dropout)
    {
     ...
     * @param center
     */
    public static ComputationGraph grabFlatEncoder(int feedDim, int baseK, int maxK, int center, double lr, int scoreDisp)
    {
        ...
        int middle = (feedDim/2) * (feedDim/2)* baseK;


        List<Triple<String, Layer, String>> inputlayers = inputFlatLayers("", maxK, baseK, 0);
        List<Triple<String, Layer, String>> centerLayers = centerFlatLayers(center, middle);
        List<Triple<String, Layer, String>> outputLayers = outputFlatLayers( baseK, maxK, true, 0);
        ...
        confBuild = confBuild.addLayer(outputLayers.get(0).getFirst(), outputLayers.get(0).getSecond(),
                new FeedForwardToCnnPreProcessor(feedDim/2,feedDim/2, baseK),outputLayers.get(0).getThird() );
    ...
    private static List<Triple<String, Layer, String>> inputFlatLayers(String combo, int maxK, int baseK, double drop)
    {
        ...
        layerList.add(new Triple<>(layerName + lyrCnt, new ConvolutionLayer.Builder().kernelSize(3,3).padding(1,1).stride(1,1).nIn(start).nOut(baseK + (start-baseK)/2).dropOut(drop).activation(Activation.LEAKYRELU).build(), startName));
        startName = layerName + lyrCnt; lyrCnt++;
        //layerList.add(new Triple<>(layerName + lyrCnt, new ConvolutionLayer.Builder().kernelSize(3,3).padding(1,1).stride(1,1).nIn(512).nOut(512).dropOut(drop).activation(Activation.LEAKYRELU).build(), startName));
        //startName = layerName + lyrCnt; lyrCnt++;
        layerList.add(new Triple<>(layerName + lyrCnt, new SubsamplingLayer.Builder().kernelSize(2,2).stride(2,2).poolingType(SubsamplingLayer.PoolingType.MAX).build(), startName));
        ...
        layerList.add(new Triple<>("final_1", new ConvolutionLayer.Builder().kernelSize(3,3).padding(1,1).stride(1,1).nIn(baseK + (start-baseK)/2).nOut(baseK).dropOut(drop).activation(Activation.LEAKYRELU).build(), startName));
    ...
    //128 = 8x4x4
    private static List<Triple<String, Layer, String>> centerFlatLayers(int centerDim, int middleDim)
    {
        List<Triple<String, Layer, String>> layerList = new ArrayList<>();

        layerList.add(new Triple<>("center_in", new DenseLayer.Builder().nIn(middleDim).nOut(centerDim)
                .weightInit(WeightInit.XAVIER).activation(Activation.LEAKYRELU).build(), "final_1"));

        layerList.add(new Triple<>("center_out", new DenseLayer.Builder().nIn(centerDim).nOut(middleDim)
                .weightInit(WeightInit.XAVIER).activation(Activation.LEAKYRELU).build(), "center_in"));
    ...
    private static List<Triple<String, Layer, String>> outputFlatLayers(int baseK, int maxK, boolean includeLoss, double dropout)
    {
        ...
        layerList.add(new Triple<>("gen_2", new ConvolutionLayer.Builder() .kernelSize(3,3).padding(1,1) .stride(1,1).nIn(baseK).dropOut(dropout).nOut(((maxK-baseK)/2) + baseK ).activation(Activation.LEAKYRELU).build(), "center_out"));
        //layerList.add(new Triple<>("gen_2", new ConvolutionLayer.Builder().kernelSize(3,3) .stride(1,1).nIn(512).nOut(512).activation(Activation.LEAKYRELU).build(), "gen_1"));
        ...
        //layerList.add(new Triple<>(layerName + lyrCnt, new ConvolutionLayer.Builder().kernelSize(3,3).padding(1,1).stride(1,1).nIn(512).dropOut(dropout).nOut(512).activation(Activation.LEAKYRELU).build(), startName));
        //startName = layerName + lyrCnt; lyrCnt++;
        layerList.add(new Triple<>(layerName + lyrCnt, new ConvolutionLayer.Builder().kernelSize(3,3).padding(1,1).stride(1,1).nIn(((maxK-baseK)/2) + baseK ).dropOut(dropout).nOut(maxK).activation(Activation.LEAKYRELU).build(), startName));
        startName = layerName + lyrCnt; lyrCnt++;


        /**   -   **/

        layerList.add(new Triple<>("toRGB", new ConvolutionLayer.Builder().kernelSize(1,1) .stride(1,1).nIn(maxK).nOut(3).activation(Activation.IDENTITY).build(), startName));
        //layerList.add(new Triple<>("toRGB", new ConvolutionLayer.Builder().kernelSize(1,1) .stride(1,1).nIn(512).nOut(3).activation(Activation.IDENTITY).build(), "gen_2"));
        if (includeLoss) {
            layerList.add(new Triple<>("out", new CnnLossLayer.Builder().activation(Activation.IDENTITY).lossFunction(LossFunctions.LossFunction.MSE).build(), "toRGB"));
        }
